...
public class WorkshopRepositoryImpl implements WorkshopRepository {
    ...
    public List<WorkShop> getWorkshops() {
        try {
            Map<String, String> registrationIdsByWorkshop = syncMyRegistrationState();
            ...
            for (WorkshopResponseDto dto : response) {
                workshops.add(mapWorkshop(dto, registrationIdsByWorkshop.get(dto.getId())));
    ...
    public WorkShop getWorkshopById(String id) {
        try {
            ...
            return mapWorkshop(dto, registrationIdsByWorkshop.getOrDefault(id, findLocalRegistrationId(id)));
        } catch (Exception e) {
    ...
    private Map<String, String> syncMyRegistrationState() {
        Map<String, String> registrationsByWorkshop = new HashMap<>();
        SharedPreferences prefs = context.getSharedPreferences("UserPrefs", Context.MODE_PRIVATE);
        if (!"STUDENT".equalsIgnoreCase(prefs.getString("userRole", ""))) {
            return registrationsByWorkshop;
        }
        ...
        try {
            ...
            List<com.example.unihubworkshop.features.workshop.data.datasource.RegistrationDetailResponseDto> registrations =
                    registrationApi.getMyRegistrations().execute().body();

            if (registrations == null) {
                return readRegistrationPrefs();
            }
            ...
            for (com.example.unihubworkshop.features.workshop.data.datasource.RegistrationDetailResponseDto registration : registrations) {
                ...
                String workshopId = registration.getWorkshop() == null ? null : registration.getWorkshop().getId();
                if (isBlank(workshopId) || isBlank(registration.getId())) {
                    continue;
                }
            ...
            registrationsByWorkshop.putAll(readRegistrationPrefs());
        }
        return registrationsByWorkshop;
    }

    private Map<String, String> readRegistrationPrefs() {
    ...
    private boolean isActiveRegistration(com.example.unihubworkshop.features.workshop.data.datasource.RegistrationDetailResponseDto registration) {
        if (registration == null || isBlank(registration.getStatus())) {
            return false;
        }
        return "PENDING".equalsIgnoreCase(registration.getStatus())
                || "SUCCESS".equalsIgnoreCase(registration.getStatus())
    ...
    public androidx.lifecycle.LiveData<List<com.example.unihubworkshop.features.workshop.data.local.entity.RegistrationEntity>> getRegistrationDetails(String workshopId) {
        return com.example.unihubworkshop.features.workshop.data.local.db.AppDatabase.getInstance(context)
                .registrationDao().getCheckinEligibleRegistrations(workshopId);
    }
    ...
    public CheckinResult verifyOfflineCheckinDetailed(String qrCode, String workshopId) {
        ...
        if (!"SUCCESS".equals(registration.status)) {
            return CheckinResult.INVALID_TICKET;
        }

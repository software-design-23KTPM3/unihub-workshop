...
public class PaymentSandboxController {
    ...
    String controls = completed
        ...
        : """
            ...
            <form method="post" action="/sandbox/checkout/%s/server-fail">
              <button class="danger" type="submit">Server thanh toán lỗi (open circuit breaker)</button>
            </form>
  ...
  public ResponseEntity<Void> success(@PathVariable String paymentId) {
    ...
    return redirectToReturnUrl(payment, "success");
  }
  ...
  public ResponseEntity<Void> fail(@PathVariable String paymentId) {
    ...
    return redirectToReturnUrl(payment, "retry");
  }
  ...
  private ResponseEntity<Void> redirectToReturnUrl(SandboxPayment payment, String result) {
    String returnUrl = payment.getReturnUrl();
    String separator = returnUrl.contains("?") ? "&" : "?";
    return ResponseEntity.status(HttpStatus.FOUND)
        .header("Location", returnUrl + separator + "paymentResult=" + result)
        .build();
  }
}
